...
export default function Dashboard(props) {
    ...
    const { data, setData, delete: destroy, post, put,
        processing, reset, errors } = useForm({
            id: '', name: '', paternal: '', maternal: ''
        });
    const openModal = (op, id, name, paternal, maternal) => {
        setModal(true);
        setOperation(op);
        setData({ name: '', paternal: '', maternal: '' });
        if (op === 1) {
            setTitle('Añadir cita');
        }
        else {
            setTitle('Modificar auto');
            setData({ id: id, name: name, paternal: paternal, maternal: maternal });
        }
    }
    const closeModal = () => {
        setModal(false);
                    ...
                    <tbody>
                        {props.cars.map((car, i) => (
                            <tr key={car.id}>
                                ...
                                <td className='border border-gray-400 px-2 py-2'>{car.paternal}</td>
                                <td className='border border-gray-400 px-2 py-2'>{car.maternal}</td>

                                <td className='border border-gray-400 px-2 py-2'>
                                    <WarningButton
                                        onClick={() => openModal(2, car.id, car.name, car.paternal, car.maternal)}>
                                        <i className='fa-solid fa-edit'></i>
                                    </WarningButton>
                                </td>
                                <td className='border border-gray-400 px-2 py-2'>
                                    <DangerButton
                                        onClick={() => eliminar(car.id, car.name, car.paternal)}>
